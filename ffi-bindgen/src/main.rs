...
fn fixup_python_lib_path(
    ...
    let bindings_file = out_dir.join("chisel.py");
...
#[structopt(
    name = "ffi-bindgen",
    about = "A tool to generate the chisel library language bindings"
)]
struct Opt {
    /// UDL file
    #[structopt(env = "FFI_BINDGEN_UDL", short, long, default_value("src/chisel.udl"), parse(try_from_str = PathBuf::from_str))]
    udl_file: PathBuf,

    /// Language to generate bindings for
    #[structopt(env = "FFI_BINDGEN_LANGUAGE", short, long, possible_values(&["kotlin","swift","python"]), parse(try_from_str = Language::from_str))]
    language: Language,

    /// Output directory to put generated language bindings
    #[structopt(env = "FFI_BINDGEN_OUTPUT_DIR", short, long, parse(try_from_str = PathBuf::from_str))]
    out_dir: PathBuf,

    /// Python fix up lib path
    #[structopt(env = "FFI_BINDGEN_PYTHON_FIXUP_PATH", short, long, parse(try_from_str = PathBuf::from_str))]
