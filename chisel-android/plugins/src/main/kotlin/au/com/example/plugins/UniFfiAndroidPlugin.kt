package au.com.example.plugins
...
internal class UniFfiAndroidPlugin : Plugin<Project> {
    override fun apply(target: Project): Unit = target.run {
        ...
        val buildAndroidAarch64Binary by tasks.register<Exec>("buildAndroidAarch64Binary") {
            ...
            workingDir("${projectDir}/../../chisel-ffi")
            ...
            doLast {
                println("Native library for chisel-android on aarch64 built successfully")
            }
        }
        ...
        val buildAndroidX86_64Binary by tasks.register<Exec>("buildAndroidX86_64Binary") {
            ...
            workingDir("${project.projectDir}/../../chisel-ffi")
            val cargoArgs: MutableList<String> =
            ...
            doLast {
                println("Native library for chisel-android on x86_64 built successfully")
            }
        }
        ...
        val buildAndroidArmv7Binary by tasks.register<Exec>("buildAndroidArmv7Binary") {
            ...
            doLast {
                println("Native library for chisel-android on armv7 built successfully")
            }
        }

        // move the native libs build by cargo from chisel-ffi/target/<architecture>/release/
        // to their place in the chisel-android library
        ...
        val moveNativeAndroidLibs by tasks.register<Copy>("moveNativeAndroidLibs") {
            ...
            into("arm64-v8a") {
                from("${project.projectDir}/../../target/aarch64-linux-android/release/libchiselffi.so")
            }

            into("x86_64") {
                from("${project.projectDir}/../../target/x86_64-linux-android/release/libchiselffi.so")
            }

            into("armeabi-v7a") {
                from("${project.projectDir}/../../target/armv7-linux-androideabi/release/libchiselffi.so")
            }
        ...
        val generateAndroidBindings by tasks.register<Exec>("generateAndroidBindings") {
            ...
            args(
                ...
                "kotlin",
                "--out-dir",
                "../chisel-android/lib/src/main/kotlin"
            )
